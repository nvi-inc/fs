 ...
 */
/***************************************************************************\
* Modified by Ed Himwich (WEH), NVI/NASA/GSFC in April 2012 for use in FS   *
*****************************************************************************
...
/* #include "predict.h"   removed and replaced with this dummy information WEH 120416 */
/* This file was generated by the installer program */

char *predictpath={"/usr2/fs/predictFS/"}, soundcard=0, *version={"2.2.3"};
...
int x;
{
	...
	sprintf(string,"%05d",sat[x].catnum);
	...
	sprintf(string,"%4u",sat[x].setnum);
	...
	sprintf(string,"%5u",sat[x].orbitnum);
...
char ReadDataFiles()
{
	...
	if (fd!=NULL)
	{
	  /* WEH 120418 detect read errors or empty qth file */

	        if(NULL == fgets(qth.callsign,16,fd)) {
	  printf("\n%c*** ERROR!  Your QTH file \"%s\" could not be read!\n\n",
		 7,qthfile);
		  exit(-5);
		}
		qth.callsign[strlen(qth.callsign)-1]=0;
		if(1!=fscanf(fd,"%lf", &qth.stnlat)) {
	  printf("\n%c*** ERROR!  Your QTH file \"%s\" could not be read!\n\n",
		 7,qthfile);
		  exit(-5);
		}
		if(1!=fscanf(fd,"%lf", &qth.stnlong)) {
	  printf("\n%c*** ERROR!  Your QTH file \"%s\" could not be read!\n\n",
		 7,qthfile);
		  exit(-5);
		}
		if(1!=fscanf(fd,"%d", &qth.stnalt)) {
	  printf("\n%c*** ERROR!  Your QTH file \"%s\" could not be read!\n\n",
		 7,qthfile);
		  exit(-5);
		}
...
double daynum;
{
	...
	/* WEH 120416 improve rounding */
	t=(time_t)((86400.0*(daynum+3651.0))+.5);
...
void ShowOrbitData()
{
	...
	while (x!=-1)
	{
		if (sat[x].meanmo!=0.0)
		{
			...
			printw("Orbital Data For %s / Catalog Number %d\n",sat[x].name,sat[x].catnum);
			attrset(COLOR_PAIR(3)|A_BOLD);
			...
			mvprintw(20,40,": %d",sat[x].orbitnum);
			mvprintw(21,40,": %d",sat[x].setnum);
...
void KepEdit()
{
	...
	do
	{
		...
		if (x!=-1)
		{
			...
			mvprintw(20,42,"%d",sat[x].orbitnum);
			mvprintw(21,42,"%d",sat[x].setnum);
			...
			sprintf(temp,"%d",sat[x].catnum);

			if (KbEdit(43,9))
				sscanf(temp,"%d",&sat[x].catnum);
			...
			sprintf(temp,"%d",sat[x].orbitnum);

			if (KbEdit(43,21))
				sscanf(temp,"%d",&sat[x].orbitnum);

			sprintf(temp,"%d",sat[x].setnum);

			if (KbEdit(43,22))
				sscanf(temp,"%d",&sat[x].setnum);
...
char speak;
{
	...
	do
	{
		...
		mvprintw(21,22,"Orbit Number: %d",rv);
...
char *string, *outputfile;
{
	...
	/* Do a simple search for the matching satellite name */
	/* WEH 120419 don't match empty entry when satname is not a number */
	for (z=0; z<24 && sat[z].catnum!=0; z++)
	{
	...
	/* WEH 120416 increase output precision */
					if (Decayed(indx,daynum)==0)
						fprintf(fd,"%d %s %8.4f %8.4f %4d %4d %4d %6d %6d %c\n",count,Daynum2String(daynum),sat_ele,sat_azi,ma256,isplat,isplong,irk,rv,findsun);
				}
				break;
			}
		}
...
char argc, *argv[];
{
	...
	else
	{
		if (tle_cli[0] || qth_cli[0])
		{
			...
			/* WEH 120418 more information in exit code */
			if(x==1)
			  exit(-2);
			else if(x==2)
			  exit(-3);
