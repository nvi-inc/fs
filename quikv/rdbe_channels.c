...
void rdbe_channels(command,itask,ip)
...
int ip[5];                           /* ipc parameters */
{
  ...
  if (command->equal != '=') {
    ...
    if (*command->argv[0]=='?') {
      kcom=1;
      rdbe_channels_dis(command,irdbe,ip,&rtn_class,&rtn_recs,kcom,kmon);
      ip[0]=rtn_class;
      ip[1]=rtn_recs;
      return;
    }
...
parse:
  ...
  while( count>= 0) {
    ...
    ierr=rdbe_channels_dec(&lclc,&ifc,&count,ptr);
  ...
  if(irdbe==0) {
    ...
    for(j=0;j<MAX_RDBE_IF;j++)
      if(ifc==-1||ifc==j)
        memcpy(&shm_addr->rdbe_channels[irdbe].ifc[j],&lclc.ifc[j],sizeof(lclc.ifc[j]));
  }
    ...
    if(ifc==-1||ifc==j)
      for (i=0;i<MAX_RDBE;i++)
        if(irdbe == i+1 || shm_addr->rdbe_active[i]!=0 && irdbe ==0 )
          memcpy(&shm_addr->rdbe_channels[i+1].ifc[j],&lclc.ifc[j],sizeof(lclc.ifc[j]));

  for(j=0;j<MAX_RDBE_IF;j++) {
    if(ifc==-1||ifc==j) {
      rdbe_channels_2_rdbe(outbuf,&lclc,j);
      if(outbuf[0]!=0)
        for (i=0;i<MAX_RDBE;i++)
          if(irdbe == i+1 || shm_addr->rdbe_active[i]!=0 && irdbe ==0 ) {
  ...
  for (i=0;i<MAX_RDBE;i++)
    if(irdbe == i+1 || shm_addr->rdbe_active[i]!=0 && irdbe == 0) {
      ...
      if(ip[2]<0) {
        ...
        rdbe_channels_dis(command,iwhich,ip,&rtn_class,&rtn_recs,kcom,kmon);
      if(irdbe !=0)
        continue;
