...
void rdbe_data_send(command,itask,ip)
...
int ip[5];                           /* ipc parameters */
{
  ...
  if (command->equal != '=') {
    ...
    if (*command->argv[0]=='?') {
      kcom=1;
      rdbe_data_send_dis(command,irdbe,ip,&rtn_class,&rtn_recs,kcom,kmon);
      ip[0]=rtn_class;
      ip[1]=rtn_recs;
      return;
    }
...
parse:
  ...
  memcpy(&lclc,&shm_addr->rdbe_data_send[irdbe],sizeof(lclc));

  count=1;
  while( count>= 0) {
    ptr=arg_next(command,&ilast);
    ierr=rdbe_data_send_dec(&lclc,&count, ptr);
    if(ierr !=0 ) goto error;
  }
  if(lclc.end.state.known==1 && lclc.delta.state.known==1) {
    ierr=-302;
    goto error;
  }
  ...
  if(irdbe==0) {
    ...
    memcpy(&shm_addr->rdbe_data_send[irdbe],&lclc,sizeof(lclc));
  }

  for (i=0;i<MAX_RDBE;i++)
    if(irdbe == i+1 || shm_addr->rdbe_active[i]!=0 && irdbe ==0 ) {
      memcpy(&shm_addr->rdbe_data_send[i+1],&lclc,sizeof(lclc));
    }

  rdbe_data_send_2_rdbe(outbuf,&lclc);
  if(outbuf[0]!=0)
    for (i=0;i<MAX_RDBE;i++)
      if(irdbe == i+1 || shm_addr->rdbe_active[i]!=0 && irdbe ==0 ) {
  ...
  for (i=0;i<MAX_RDBE;i++)
    if(irdbe == i+1 || shm_addr->rdbe_active[i]!=0 && irdbe == 0) {
      ...
      if(ip[2]<0) {
        ...
        rdbe_data_send_dis(command,iwhich,ip,&rtn_class,&rtn_recs,kcom,kmon);
      if(irdbe !=0)
        continue;
