...
void dbbc3_bbcnn_dis(command,itask,ip)
...
long ip[5];
{
	 ...
	 return;
         ...
         memcpy(&lclc,&shm_addr->dbbc3_bbcnn[ind],sizeof(lclc));
      else {
	for (i=0;i<ip[1];i++) {
	  if ((nchars =
	  ...
	  if(i==0)
	    ierr=dbbc3_2_bbcnn(&lclc,&lclm,inbuf);
	  if( ierr!=0) {
	    if(i<ip[1]-1) 
	      cls_clr(ip[0]);
	    ierr=-403;
   ...
   /* format output buffer */
      ...
      while( count>= 0) {
        ...
        count++;
        dbbc3_bbcnn_enc(output,&count,&lclc);
      }
	...
	while( count>= 0) {
	  if (count > 0) strcat(output,",");
	  count++;
	  dbbc3_bbcnn_mon(output,&count,&lclm);
	}
      }
