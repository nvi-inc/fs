 ...
 */
/* rdbe dot2gps and dot2pps SNAP command display */
...
void rdbe_dot2Xps_dis(command,iwhich,ip,out_class,out_recs)
...
int *out_recs;
{
  ...
  for (i=0;i<nrecs;i++) {
    ...
    if(i==0)
      if(0!=rdbe_2_rdbe_dot2Xps(inbuf,&lclm,ip)) {
        if(i<nrecs-1)
          cls_clr(iclass);
        goto error;
      }
  ...
  while( count>= 0) {
    ...
    count++;
    rdbe_dot2Xps_mon(output,&count,&lclm,iwhich);
  }
