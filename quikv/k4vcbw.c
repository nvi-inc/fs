/* k4 video converter bw snap command */
...
parse:
      ...
      memcpy(&lcl,&shm_addr->k4vcbw,sizeof(lcl));

      count=1;
      while( count>= 0) {
        ptr=arg_next(command,&ilast);
        ierr=k4vcbw_dec(&lcl,&count, ptr,itask);
...
/* all parameters parsed okay, update common */
      ...
      memcpy(&shm_addr->k4vcbw,&lcl,sizeof(lcl));
      
/* format buffers for k4con */

      k4vcbw_req_c(ip,&lcl,itask);
