...
void rdbe_pc_offset(command,itask,ip)
...
int ip[5];                           /* ipc parameters */
{
  ...
  if (command->equal != '=') {
    ...
    if (*command->argv[0]=='?') {
      kcom=1;
      rdbe_pc_offset_dis(command,irdbe,ip,&rtn_class,&rtn_recs,kcom,kmon);
      ip[0]=rtn_class;
      ip[1]=rtn_recs;
      return;
    }
...
parse:
  ...
  memcpy(&lclc,&shm_addr->rdbe_pc_offset[irdbe],sizeof(lclc));

  count=1;
  while( count>= 0) {
    ptr=arg_next(command,&ilast);
    ierr=rdbe_pc_offset_dec(&lclc,&count, ptr,irdbe);
...
// can't just use the lclc structure, overall default could imply
// differetnt values for ech RDBE, so make an overall structure to update
// and only copy it to shared memory if there are no errors

  memcpy(&rdbe_pc_offset,&shm_addr->rdbe_pc_offset,sizeof(rdbe_pc_offset));
  ...
  if(irdbe==0) {
    ...
    memcpy(&rdbe_pc_offset[irdbe],&lclc,sizeof(lclc));
  }

  ierr=0;
  for (i=0;i<MAX_RDBE;i++)
    if(irdbe == i+1 || shm_addr->rdbe_active[i]!=0 && irdbe ==0 ) {
      memcpy(&rdbe_pc_offset[i+1],&lclc,sizeof(lclc));
      if(lclc.offset.offset==-1 && lclc.offset.state.known==1) {
        double spacing=shm_addr->lo.spacing[i*2];
        double lo=shm_addr->lo.lo[i*2];

        if(lo>=0.0 && spacing>=0) {
          rdbe_pc_offset[i+1].offset.offset=spacing-fmod(lo,spacing);
          rdbe_pc_offset[i+1].offset.offset*=1e6;
        } else {
          if(ierr!=0)
            logita(NULL,ierr,"2p",ip+4);
          ierr=-302;
          who[1]=toupper(unit_letters[i+1]);
          memcpy(ip+4,who,2);
        }
      }
    }
...
// now done, copy to shared memory for posterity

  memcpy(&shm_addr->rdbe_pc_offset,&rdbe_pc_offset,sizeof(shm_addr->rdbe_pc_offset));
  for (i=0;i<MAX_RDBE;i++)
    if(irdbe == i+1 || shm_addr->rdbe_active[i]!=0 && irdbe ==0 ) {
      rdbe_pc_offset_2_rdbe(outbuf,&rdbe_pc_offset[i+1]);
  ...
  for (i=0;i<MAX_RDBE;i++)
    if(irdbe == i+1 || shm_addr->rdbe_active[i]!=0 && irdbe == 0) {
      ...
      if(ip[2]<0) {
        ...
        rdbe_pc_offset_dis(command,iwhich,ip,&rtn_class,&rtn_recs,kcom,kmon);
      if(irdbe !=0)
        continue;
