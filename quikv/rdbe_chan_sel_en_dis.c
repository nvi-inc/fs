 ...
 */
/* RDBE chan_sel_en SNAP command display */
...
void rdbe_chan_sel_en_dis(command,iwhich,ip,out_class,out_recs,kcom,kmon)
...
int kmon;
{
  ...
  if(kcom) {
    memcpy(&lclc,&shm_addr->rdbe_chan_sel_en[iwhich],sizeof(lclc));
    ...
    for (i=0;i<nrecs;i++) {
      ...
      if(i==0)
        if(0!=rdbe_2_rdbe_chan_sel_en(inbuf,&lclc,ip)) {
          if(i<nrecs-1)
            cls_clr(iclass);
          goto error;
        }
  ...
  while( count>= 0) {
    ...
    count++;
    rdbe_chan_sel_en_enc(output,&count,&lclc);
  }
