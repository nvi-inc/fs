...
void dbbcifx_dis(command,itask,ip)
...
int ip[5];
{
	 ...
	 return;
         ...
         memcpy(&lclc,&shm_addr->dbbcifx[ind],sizeof(lclc));
      else {
	for (i=0;i<ip[1];i++) {
	  if ((nchars =
	  ...
	  if(i==0)
	    ierr=dbbc_2_dbbcifx(&lclc,&lclm,inbuf);
	  if(ierr!=0) {
	    if(i<ip[1]-1) 
	      cls_clr(ip[0]);
	    ierr=-403;
	    logite(inbuf2,-402,"di");
	    goto error;
	  }
	}
      }
