 ...
 */
/* rdbe_bstate SNAP command display */
...
void rdbe_bstate_dis(command,iwhich,ip,out_class,out_recs)
...
int *out_recs;
{
  ...
  for (i=0;i<nrecs;i++) {
    ...
    if(0!=rdbe_2_rdbe_bstate(inbuf,&lclm,ip,i)) {
      if(i<nrecs-1)
        cls_clr(iclass);
      goto error;
    }
  ...
  for (i=0;i<MAX_RDBE_IF;i++) {
    ...
    while( count>= 0) {
      ...
      count++;
      rdbe_bstate_mon(output,&count,&lclm,iwhich,i);
    }
