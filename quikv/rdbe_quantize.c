...
void rdbe_quantize(command,itask,ip)
...
parse:
  ...
  if(irdbe!=0)
    ilast++;
  memcpy(&lclc,&shm_addr->rdbe_quantize[irdbe],sizeof(lclc));

  count=1;
  while( count>= 0) {
    ptr=arg_next(command,&ilast);
    ierr=rdbe_quantize_dec(&lclc,&count, ptr);
  ...
  if(irdbe==0) {
    ...
    memcpy(&shm_addr->rdbe_quantize[irdbe],&lclc,sizeof(lclc));
  }
