...
parse:
      ...
      memcpy(&lcl,&shm_addr->k4vc,sizeof(lcl));

      count=1;
      while( count>= 0) {
        ptr=arg_next(command,&ilast);
        ierr=k4vc_dec(&lcl,&ivc,&count, ptr,itask);
...
/* all parameters parsed okay, update common */
      ...
      memcpy(&shm_addr->k4vc,&lcl,sizeof(lcl));
      
/* format buffers for k4con */

      k4vc_req_c(ip,&lcl,ivc,itask);
