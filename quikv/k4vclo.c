...
parse:
      ...
      memcpy(&lcl,&shm_addr->k4vclo,sizeof(lcl));

      count=1;
      while( count>= 0) {
        ptr=arg_next(command,&ilast);
        ierr=k4vclo_dec(&lcl,&ivc,&count, ptr,itask);
...
/* all parameters parsed okay, update common */
      ...
      memcpy(&shm_addr->k4vclo,&lcl,sizeof(lcl));
      
/* format buffers for k4con */

      k4vclo_req_c(ip,&lcl,ivc,itask);
