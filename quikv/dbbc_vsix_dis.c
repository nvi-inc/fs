...
int ip[5];
{
	 ...
	 return;
         ...
         memcpy(&lclc,&shm_addr->dbbc_vsix[itask],sizeof(lclc));
      else { /*not used yet since the device gives no monitor response */
	     /*not coded yet, just left overs here */
	//	for(i=0;i<16;i++)
	//  lclc->core[i]=0;
	...
	//  if(i==0)
	//    ierr=dbbc_2_dbbc_vsix(&lclc,inbuf);
	//  if(ierr!=0) {
	//    if(i<ip[1]-1) 
	//      cls_clr(ip[0]);
	//    ierr=-403;
   ...
   /* format output buffer */
      ...
      while( count>= 0) {
        ...
        count++;
        dbbc_vsix_enc(output,&count,&lclc);
      }
