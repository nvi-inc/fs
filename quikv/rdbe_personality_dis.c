 ...
 */
/* RDBE personality SNAP command display */
...
void rdbe_personality_dis(command,iwhich,ip,out_class,out_recs,kcom,kmon)
...
int kmon;
{
  ...
  if(kcom) {
    memcpy(&lclc,&shm_addr->rdbe_personality[iwhich],sizeof(lclc));
    ...
    for (i=0;i<nrecs;i++) {
      ...
      if(i==0)
        if(0!=rdbe_2_rdbe_personality(inbuf,&lclc,&lclm,ip)) {
          if(i<nrecs-1)
            cls_clr(iclass);
          goto error;
        }
  ...
  while( count>= 0) {
    ...
    count++;
    rdbe_personality_enc(output,&count,&lclc);
  }
  if (!kcom) {
    count=0;
    while( count>= 0) {
      if (count > 0) strcat(output,",");
      count++;
      rdbe_personality_mon(output,&count,&lclm);
    }
  }
