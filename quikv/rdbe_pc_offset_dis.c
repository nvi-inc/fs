 ...
 */
/* rdbe_pc_offset SNAP command display */
...
void rdbe_pc_offset_dis(command,iwhich,ip,out_class,out_recs,kcom,kmon)
...
int kmon;
{
  ...
  if(kcom) {
    memcpy(&lclc,&shm_addr->rdbe_pc_offset[iwhich],sizeof(lclc));
    ...
    for (i=0;i<nrecs;i++) {
      ...
      if(i==0)
        if(0!=rdbe_2_rdbe_pc_offset(inbuf,&lclc,ip)) {
          if(i<nrecs-1)
            cls_clr(iclass);
          goto error;
        }
  ...
  while( count>= 0) {
    ...
    count++;
    rdbe_pc_offset_enc(output,&count,&lclc);
  }
