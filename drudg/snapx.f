...
C
      ...
      include 'recorder.ftni'           !This contains info only about the recorders.
...
! JMG variables
      ...
      integer itime_pass_beg(5)             !time we start a pass.
!
      ...
      integer itime_pass_beg_prev(5)        !time we started last pass
      integer itime_delta                   !Difference between two times in seconds
!      integer itime_cal_prev(5)
...
9102    format(/' SNAP01 - Rack or recorder type is unknown. Please',
     ...
     .   kv4rec,ks2rec,kk41rec,kk42rec,km5rec)
      km5_piggyback=kmk5_piggyback         !this is needed for 'recorder.ftni'
...
C     check to see if the file exists first
      ...
      inquire(file=snpname,exist=kexist,iostat=ierr)
      if (kexist) then
        if (kbatch) then
          response='Y'
        else
...
C
      ...
      istnsk=0
      ntape=0
      itime_scan_beg_prev=0
      itime_scan_beg_next=0
      itime_pass_beg_prev=0
      itime_pass_beg=0
...
C         Force new tape on the first scan on tape.
          ...
          if(iobsst .gt. 0) then
            itime_early_prev	=itime_early
            itime_scan_beg_prev	=itime_scan_beg
            itime_scan_end_prev	=itime_scan_end
            itime_late_prev	=itime_late
          endif
     ...
     >          ls2speed(1,istn),ibuf)
        ...
        write(123,'(2(i2.2,":"),i2.2,1x,i3,1x,4a2,1x,a2,2i8,f8.2)')
     >      itime_scan_Beg(3),itime_Scan_beg(4),itime_Scan_beg(5),
     >      idur(istnsk),
     >      LSORNA(1:4,ISOR),
     >      ldir(istnsk),
     >      ift(istnsk),ift(istnsk)+idur(istnsk)*speed_inches/12.*idir

          kNewPass = (ipasp.ne.ipas(istnsk))
          if(kNewPass) then
             itime_pass_beg_prev=itime_pass_beg
             itime_pass_beg=itime_scan_beg
          endif
...
C     kstop means to issue the ET at the usual place, before POSTOB
        kstop_pre=kstop ! previous scan
        kstop=.false.

        if(kadap) then
          if(ks2) then          !s2
            if (iobsst.gt.0) then ! got a previous scan
!              call TimeAdd(itime_scan_end_prev,itlate(istn),itime_temp)
C             idt = tape start - previous tape stop the gap.
...
C Use this section only for continuous
          ...
          ilatestop=0
          ket = .false.
          if (iobsst.gt.0.and..not.kstop_pre) then ! calculate new times based
...
! If sked calculated footings correctly, this is what we would do:
              if(.false.) then
...
!             iftrem= feet remaining on the pass from the ending footage of last scan
                ilatestop = max(ifix(float(iftrem)/speed(icod,istn)),0)
              else
! But since sked makes slight errors that build up over time, this is what
! we do to try to keep in sync with schedule.
                iTime_Delta=iTimeDifSec(itime_scan_beg,
     >               itime_scan_end_prev)

                itemp=(ift(istnsk)-iftold)/speed(icod,istn)

                if(idirp .eq. 1) then
                   ilatestop=iTime_Delta+itemp
                else
                   ilatestop=iTime_Delta-itemp
                endif
                ilatestop=ilatestop/2
              endif
              ilatestop=max(1,ilatestop)
              write(*,'("ft, sec:",4i8)') ift(istnsk),iftold,
     >             iTime_Delta,ilatestop
C             time6 = scheduled start time + late stop to reach end of tape
              itime_Delta=iTimeDifSec(itime_scan_end_prev,
     >                itime_pass_beg_prev)
              itemp=itime_Delta*speed_ft
              itemp=nint(float(ift(istnsk)-itemp)/speed_ft)

              call TimeAdd(itime_scan_end_prev,ilatestop,itime_pass_end)
              itime_tape_start=itime_scan_beg
...
C scan_name command. 
        ...
        write(lu_outfile,'("scan_name=",a)') scan_name(iobs_now)(1:nch)
C SOURCE command
        IOBSP = IOBSP+1
...
!          if (.not.ks2.and..not.kk4.and..not.km5.or..not.kcont) then
         if (.not.ks2.and..not.kk4.and..not.km5.and..not.kcont) then
...
C Switch recorders if we have two of them, and they are both used.
     ...
     >          ((ichcm_ch(lstrec(1,istn),1,'unused').eq.0 .or.   !If either one is idle, then don't switch
     >           ichcm_ch(lstrec2(1,istn),1,'unused').eq.0))) then
              if (crecorder.eq.'1') then
                crecorder='2'
                kk4 = kk41rec(2).or.kk42rec(2)
                ks2 = ks2rec(2)
                krec = ichcm_ch(lstrec2(1,istn),1,'none').ne.0
...
C POSTOB
        call snap_hol_wrt(lu_outfile,lpst,nch)
