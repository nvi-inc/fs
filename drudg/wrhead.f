	SUBROUTINE wrhead(lu,ierr,cs,iw,isig,idoub,ldoub,ido,imode)
C
...
C  INPUT:
	...
	integer idoub(max_vlba,14)  ! two dimension array
	integer*2 ldoub(max_vlba,14)  ! two dimension array
...
C  loop on the number of channels read from schedule file
	  ...
	  if (ido.eq.2) then
            ...
            nch = nch + ib2as(idoub(nvset,ix),ibuf,nch,ileft+2)
	  else if (ido.eq.4) then
            if (ichcm_ch(ldoub(nvset,ix),1,'U').eq.0) then !U/L
              if (im.eq.1) nch = ichmv_ch(ibuf,nch,'U')
              if (im.eq.2) nch = ichmv_ch(ibuf,nch,'L')
            else
	      nch = ichmv(ibuf,nch,ldoub(nvset,ix),1,1)
            endif
	  end if
