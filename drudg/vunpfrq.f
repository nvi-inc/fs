...
! 2006Nov18 JMGipson. Changed lsg, lsb to ASCII
C
C  INPUT:
...
C  LOCAL:
      ...
      integer j,ic,nch
! functions
      integer fvex_double,fvex_len,fvex_int,fvex_field,fvex_units
      integer ptr_ch,fget_all_lowl
C
C  Initialize.
C
      nchandefs=0
...
C
     ...
     >     ptr_ch('FREQ'//char(0)),itemp)
        itemp=0            !set this to zero for the next call.
        if(iret .ne. 0) return
