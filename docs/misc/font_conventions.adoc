...
Some explanatory text probably follows. The anchor above is in double
square brackets `+++[[<i>anchor</i>]]+++`. The reference, with anchor
text, is in double angle brackets
`+++<<<i>anchor</i>,<i>text</i>>>+++`.

Somewhere else in the document, before or after the anchor, include a
reference to it. In this example, see <<check_files,Check files>>,
above.
...
correct relative path to the document. Including a reference to
