...
CAUTION: The function of this variable is different than the
similarly named `FS_LO_ANTCN_MODE`.

Nominal value is a local _antcn_ mode, i.e., an integer greater than 99.

If set, the value is taken as the mode to run _antcn_ with by the
`antcn_lo` command to configure all the LOs in one step (after all
`lo=...` commands in an IF setup procedure) rather than one-at-a-time
by `lo=...` commands, which use `FS_LO_ANTCN_MODE`. The `antcn_lo`
command can be placed in the `ifd_local` procedure, usually found in
the `station` SNAP procedure library. As of FS _10.3_, a call to
`ifd_local` is added to the end of the IF setup procedures,
`ifd__xx__`, generated by _drudg_. It can be added manually to IF
setup procedures written by hand.

This variable replaces the slightly different
`FS_LO_CONFIG_ANTCN_MODE` variable that was introduced in
_10.0.0-beta3_, December 2020. The new variable was introduced in FS
_10.3_.

=== FS_LO_ANTCN_MODE

CAUTION: The function of this variable is different than the
similarly named `FS_ANTCN_LO_MODE`.
