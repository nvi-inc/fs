...
If you are using the display server, to set or change the value of an
environment variable, it is necessary to reboot, or after terminating
the FS, to stop _fsserver_ with:

  fsserver stop

Then the environment value can be set and the FS FS restarted.
