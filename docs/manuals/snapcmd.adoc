...
Comments for the available BBCs depending on BBCs/IF and IFs in rack.
...
`g`, or `h`. See the Comments for the default values. This parameter
does not affect equipment set-up, but does affect the center frequency
Tcal, and FWHM values for Tsys, pointing, and SEFD measurements.
...
`65535`. See the Comments for information about the nominal operating
level.
