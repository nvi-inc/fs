...
using the FSL11 installation document,
https://nvi-inc.github.io/fsl11/. Testers of previous development
versions can follow the instructions here; some may have changed from
previous versions.
...
There are two issues that may neeed to be address (as _prog_), shared
memory variables and use of the _lognm_ program:
...
=== Set DBBC3 related environment variables
...
installed. A full discussion of the variables can be found in the
<<../1/dbbc3_ops.adoc#_dbbc3_specific_environment_variables,DBBC3
specific environment variables>> appendix of the
<<../1/dbbc3_ops.adoc#,FS DBBC3 Operations Manual>>. The section
<<../1/dbbc3_ops.adoc#_determining_what_values_to_use,Determining
what values to use>> in particular may be helpful.
...
wrap and should be easier to read.
...
change. If you have the alias _rlxr_ defined to reload the X11
...
This will prevent history entries from being changed if they are
edited and then not used. Their changing in the way can be quite
frustrating, but it is up to you if you want to prevent it. In FSL11,
the default is to prevent it (this is installed by the `make install`
step for the FS). If you are using FSL10 or FSL9 (but not FSL8), you
can prevent it by creating the file:
...
display client (or restart the FS if your are not using the display
server)
...
correcting, you may not need it. That, and the fact that are very few
users of this script, is why this change is optional. See the
...
_Harper's_ magazine's _Findings_ articles), more details follows.
...
. Add environment variables to change the handling of DBBC3 data.
...
more modern systems usually have more display real estate that should
be used.
...
. Allow setting a default number of samples for the DBBC and DBBC3
`cont_cal` commands

+

Like the device parameters _after_ the first (`_state_`, i.e., `on` or
`off`), the command for both devices will remember the previous
setting of this FS internal parameter as the default. This is intended
to facilitate using the `cont_cal` command as generated by _drudg_ in
the setup procedures by allowing a default to be set for this
parameter in the `initi` procedure at FS start.
...
useful for getting a static display of the current T~sys~.
...
.. Correct DBBC3 `bbc_gain` command error messages.
...
Clarify that the DBBC3 sends the continuous calibration control signal
to the receiver.

+

Thanks to Jon Quick (HartRAO) for this information.
...
.. Improve `help` page for `bbc_gain` for DBBC2/DBBC3.
