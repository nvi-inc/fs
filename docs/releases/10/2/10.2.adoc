...
feature suggestions and testing for DBBC3 support. These include
...
made an option. Several improvements were made to _monit7_. The order
of commands for DBBC3 syncing was changed. The condition `ddc` was
added to the `if` command for the DBBC3. An error message for the
Core3H boards having different times was added. The multicast time-out
detection was changed to an adaptive scheme. Multicast time-out error
reports were changed to a summary format. The order of TPI values for
...
down when the FS is terminated. Cleanup _fs_ and _fsclient_
command-line options. An environment variable was added to disable
needing to confirm termination in _oprin_.
...
Although it not recommended, if you don't want this prompt, you can
set the variable to `off`. For all other values, including it not
being set, the prompt will be used. The setting of this variable in
the session that started the client, with either the _fs_ or
_fsclient_, determines the behavior of _oprin_ for that client.
...
used, which will include both (plots can be further filtered with the
`-g`/`-G` options).
...
About 19 additional small features were changed or added. These
...
digits to T~cal~ value in the `#fivpt#fivept` record. Accept `waiting`
response from RDBE `dbe_data_send?` query in _fmset_. Log DBBC3
negative T~sys~ values. Add verbose option for _curl_/_wget_ in
_fesh_. Improve plog. Add a comment to a recovered log and make two
minor related improvements. Improve recommended NTP configuration.
Prevent problems with _erchk_ when the display server is not in use.
Use `set{nbsp}revert-all-at-newline on` to prevent old commands in the
history from being altered. Add script for calculating
...
About 19 small bugs were fixed. All were fairly minor but one could
...
_fivpt_ and _onoff_ with continuous calibration. Improve _fivpt_
...
reporting in _onoff_. Correct legacy T~sys~ error messages to support
four character device names. Correct DBBC3 `bbc_gain` first parameter
error messages. Correct the DBBC error message for an incorrect
detector device in T~sys~ related commands. Fix the home directory
permissions for AUID accounts. Fix the _refresh_spare_usr2_ scripts
for FSL6-8 and FSL9.
...
About 11 small improvements and expansions were made to the web pages.
...
Manual.`" Add update notes for FS _10.0.1_ and _10.1.1_ patch
releases. Improve FS _10.1.0_ update notes. Add a *TIP* for managing
directory names of FS installations from archives. Add a *TIP* for how
to avoid losing your place in a "`Details`" toggle when following a
link. Add explanation of how to navigate to a link in a "`Details`"
toggle in a different document. Expand "`Font Conventions`" document.
Update instructions for making the `gh-pages` documentation.
...
* Transfer logs directly to a reference copy.
