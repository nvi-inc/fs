...
<<fs_misc,FS miscellaneous changes>>, open the "`Details`" toggle
...
. Add support for `formbbc` and `formif` device mnemonics for DBBC3 T~sys~
related commands.
...
. Include spot calibration results in the DBBC3 T~sys~ monitor display
window,
...
. Add script, _govert_, to calculated stem:[\mathit{\frac{G}{T}}] from
SEFDs (closing https://github.com/nvi-inc/fs/issues/197[#197]).
...
About 40 small changes are covered under this item. They are roughly
...
Now it prints a warning and then exits after 10 seconds.
