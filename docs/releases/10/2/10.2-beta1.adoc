...
firmware is now recognized, information on IF target levels was added
(see the <<../1/dbbc3_ops.adoc#_if_target_levels,IF target levels>>
section of the <<../1/dbbc3_ops.adoc#,FS DBBC3 Operations Manual>> for
the details), environment variables were added to modify the use of
TPI values and control other features, support for spot
(noncontinuous) calibration was completed, and averaging/filtering of
continuous calibration data was added.
...
<<fs_bugs,FS miscellaneous bug fixes>>, open the "`Details`" toggle
...
. Expand DBBC3 support

+

Support was added for the DBBC3 personality DDC_E. Environment
variables were added to change the handling of DBBC3 TPI data.
Support was completed for DBBC3 non-continuous (_spot_) calibration.
The `tpicd=tsys` command was enabled. Add option to not command
parameters of `cont_cal` command. Add DBBC3 `ddc` as a condition for
the `if` command. The order of commands for DBBC3 syncing was changed.
...
.. Add averaging of continuous DBBC3 T~sys~ data.
...
If the LO and its polarizaion are defined for the displayed IF, the
polarization will be shown as `(L)` or `+++(R)+++`.

+

NOTE: Following the usual alphabetical order convention, `L`=`H`=`X`
and `R`=`V`=`Y`. You have to know which system you are on Left/Right,
Horizontal/Vertical, or X/Y.
...
output was improved. The version was bumped to _2.3_. Some
...
as the `_state_` now allows setting the defaults without commanding
the device.
...
stem:[\mathit{\frac{G}{T}}] from SEFD, using a very simple
...
correct target levels. Change the order of TPI values for DBBC3
`iftp__X__` commands to `_on_` then `_off_`. Improve the `help` pages
for `cont_cal` for the DBBC and DBBC3. Improve the `help` page for
`cont_cal` for the DBBC3. Improve `help` page for `bbc_gain` for DBBC2
and DBBC3. Fix `-help` command line option for _gnplt_. Improve
`logpl` help contents for the Main screen. Improve `help` page for the
`setup_proc` command.
...
.. Add information to the DBBC3 `if__X__` command `help` page for
the correct `_target_` levels.
...
FS _10.1.0_ updates to recognize that the default branch may already
