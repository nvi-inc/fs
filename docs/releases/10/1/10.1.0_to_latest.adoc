...
This document is up to date with the `11029477` commit. Due to small
...
=== Fetch and make the latest commit and station code
...
. If needed, set new environment variables

+

For FSL11 no new environment variables need to be set, skip the
remainder of this step.

+

For FSL__x__ Linux distributions FSL10 and older, set the environment
variables `FS_F95_NO_ALLOW_ARGUMENT_MISMATCH` and `FS_PYTHON_VERSION`.
Some other non-FSL__x__ distributions may need one or both set as
well.
...
This change is largely transparent to the users. The three visible
effects are:
...
(such as _telegraf_ for the MAS) that are pinned to the FS _10.0_
...
_not_ in use. However, it would be safer to use the log output (no
`-d`) instead. If it is used without the display server enable, it
will crash the FS immediately after start-up.

* The `-h` option is not useful in _stpgm.ctl_. Its use will cause the
FS to crash immediately after start-up.

* The `-s` option can be used in _stpgm.ctl_ but is generally not
useful unless the display server is not enabled. Even then it is of
marginal value.
