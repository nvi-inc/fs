...
This update is intended for all stations using FS _10.0.0_. This
document covers the steps needed to update from FS _10.0.0_ to
_10.1.0_. The changes in the FS and _drudg_ for _10.0.0_ are covered
in the <<Changes since 10.0.0>> section below.
...
Installing the new version as an update is much simpler than upgrade
to _10.0.0_, see the <<Upgrading from 10.0.0>> section below.

The most significant changes in _10.0.0_ are:

* FS and _drudg_ support for DBBC3 racks.

* Use of command and procedure with names containing `fb` instead of
`mk5`. For FS commands the new names are synonynms for the old names.

* Use of `mk5c_config` nd `fb_config` procedures for local tuning of
Mark 5C and FlexBuff recorders.

* Instrumentation record _fsserver_ errors.

* Change the default FS git branch to `main`

NOTE: This has not been implemented in the `alpha` pre-releases.

* Numerous small enhancements and fixes.

For the complete list of changes, see the <<Changes since 10.0.0>>
sections below.
...
=== Fetch and make FS 10.1.0 and make station code

CAUTION: With this release, we have changed the default _git_ branch
for the FS. This will be transparent for all users unless they have
made local changes to their FS _git_ repo. If you have made such
changes and need to preserve them, please contact Ed for methods to
handle this.
...
* If you are are on an FSL9 or FSL10 system:
...
change to `refrwn()` in the long term. Please see the
...
in your old _equip.ctl_ file. That should allowing running the FS if
you don't have a DBBC3. If you have a DBBC3, you should customize the
contents in the <<dbbc3config,FS DBBC3 Configuration>> step below.
...
Alternatively, if the FS is not running, you can add empty procedures
to your `station` procedure library with the following commands. For
`mk5c_config`:
...
subsection of the <<dbbc3_ops.adoc#,FS 10.1 DBBC3 Operations Manual>>
...
<<dbbc3_ops.adoc#_configuring_the_fs_for_a_dbbc3,Configuring the FS
for a DBBC3>> appendix of the <<dbbc3_ops.adoc#,FS 10.1 DBBC3
Operations Manual>>. If you expect to get a DBBC3, these changes can
...
support>> document for release, _10.0.0_. More complete support is
...
details of DBBC3 functionality; Uwe Bach (Effelsberg), Marjolein
...
command provides full functionally for setting the sample rate for
