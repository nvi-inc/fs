...
_10.1.0_ and the changes in new version. This update is intended for
all stations using FS _10.0.0_. Installing the new version as an
update is much simpler than the update to _10.0.0_, see the
<<Upgrading from 10.0.0>> section below.
...
* Use of command and procedure with names for FlexBuff recorders
containing `fb` instead of `mk5`. For FS commands the new names are
synonyms for the old names.
...
* Improved presentation of the list of FS and _drudg_ changes in the
update notes (this document) to show a summary line and a _toggle_ to
open a more detailed description. This should make the list of changes
easier to review while allowing a reader to view more details for
items they find interesting.

* Instrumentation to record any _fsserver_ errors that may occur.
...
symbolic links>> subsection of above referenced section.
...
10.1 DBBC3 Operations Manual>>. If you expect to get a DBBC3, these
...
* There is extra white space when the toggle is closed.

* If you return "`back`" to the document after following a link to a
_different_ document from within opened text, the previously opened
text will be closed because the web page has been reloaded.
