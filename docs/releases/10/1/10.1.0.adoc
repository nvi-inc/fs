...
confusion later if you have a DBBC3, or get one, since the racks types
for DBBC3s have changed. You can use the example/default file in
_/usr2/fs/st.default/control/equip.ctl_ for comparison.

+

IMPORTANT: If you have a DBBC3, you _will_ need to update your rack
type, but this is covered below in the <<dbbc3config,FS DBBC3
Configuration>> step.
...
DBBC3 Operations Manual>>. You may have already made some of these
changes, but you should verify them all, particularly setting the
correct rack type. If don't have a DBBC3, but expect to get one, these
...
* New DBBC3 rack types `dbbc_ddc_u` and `dbbc3_ddc_v` replace `dbbc3`
in _equip.ctl_.
