...
Except for the `Time` field, the displayed values are from the
previous second's multicast. Hence the T~sys~ values are from two
seconds in the past. If the system is operating normally, the `Time`
field shows a value one second more than in the previous second's
multicast to avoid confusion with other displayed time fields. (Logged
values of the time are the raw received values.) This leads to the
somewhat odd situation that for a Core3H board that is not synced,
with firmware version _v125_ or higher, its time will be shown as
`00:00:01` on the first day of the current VDIF epoch.
...
board's time in the multicast is `00:00:00` on the first day of the
current VDIF epoch (so twice a year for about one second each, it is
possible for _setcl_ to incorrectly think the board is not synced). A
lack of sync should also be shown as `unsynced` in the `core3h_mode`
...
usage. We believe we that there will be no "`extra`" errors caused by
DBBC3 commands. However, we cannot be sure every case has been caught.
There is some chance that there will be extra errors reported  1 to 3
seconds after the most recent DBBC3 communication. Please report this
situation if you encounter it, so it can be fixed. It is more
...
* Up to eight IFs (`a`-`h`) with up to 16 dual side-band BBCs each
(overall `001`-`128`) for VEX (_.vex_) schedule files.

* Up to two IFs (`a` and `b`) with up to eight 8 dual side-band BBCs
(`001`-`008`) on IF `a` and up to eight 8 dual side-band BBCs
...
NOTE: Setting the new Core3H payload size _outside_ of the overall
setup procedure is so using that procedure without `force` to check
the DBBC3 configuration (and/or to configure the non-Core3H parts of
the system), the added ``core3h=__n__,vdif_frame ...`` commands won’t
trigger a requirement to re-sync the boards.
