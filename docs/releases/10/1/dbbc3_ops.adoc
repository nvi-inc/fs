...
:experimental:
E. Himwich, J. Quick
Version 4.6 - April 2023

== Introduction

This document describes FS DBBC3 support starting with FS _10.1_. It
outlines the features and model for FS operations with the DBBC3. It
contains a section <<Configuring the FS for use with a DBBC3>> that
gives the steps for customizing the FS.
...
The appendix <<DBBC3 specific environment variables>> explains the use
of those variables. The appendix
<<Verifying polarity and USB/LSB TPI swaps>> provides methods for
checking those two specific issues.
...
support a DBBC3. You must have version FS _10.1_, or later, installed
before using these directions. All steps, except as noted, are to be
executed as _oper_.
