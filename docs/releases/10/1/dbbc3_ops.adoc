...
are also looking into how to support `scan_check` for single thread
per file.
...
PPS and sync each Core3H. In principle, this would consist of:
...
commands for boards you have. It may be necessary to increase the
delay after the `pps_sync` command to achieve reliable results.
