...
NOTE: in FS _10.1_, if one IF was manually selected before the IF and
BBC configuration was defined (or at startup of the display window
before any IF had been displayed), it will not be possible to toggle
between display modes until cycling has been restarted. This was fixed
in _10.2_.
...
the output of the `mcast_time` command, and optionally  _setcl_. The
system will need to be synced to NTP and the `computer` model selected
in _time.ctl_. In this case, the last parameter, `hsecs`, on the
`mcast_time/0` line _minus_ the last value on the `#setcl#model/old`
line is the value to put in the file; it may vary at the single digit
centisecond level. If _setcl_ it not being used, the value from it can
...
* The arrival time of the message -- is represented by the time-tag of
the log entry with centisecond precision.

+

The fractional seconds should agree with the last value, `hsecs`, on
the `mcast_time/0` line with small variations at the one centisecond
level.
