...
the directions in the <<note,NOTE>> below in this step.

====

There are three options given below. The directions in the first two
options include preserving a copy of your current _/usr2/fs-git_
directory. This is optional, but may help you recover if there is an
error. You can of course use any name you like for the copy. You
should use a different name for the copy than _fs-git.old-10.0.0_ if
...
. As _root_, make a copy of your current _/usr2/fs-git_ directory;
...
successfully. It is also not needed when the display server is in use.
...
details of items that interest you. The summary sentences are omitted
if they would not add any new information, usually because it is
already covered in the numbered title item and/or the details are very
brief.
...
.. Locate references for `refrwn()` and `refr()`
...
DBBC2/FlexBuff were added. A new exmple was added for DBBC3/Flexbuff.
