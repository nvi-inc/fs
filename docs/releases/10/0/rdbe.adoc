...
_.Xresources_ for _oper_ and _prog_.
...
or `Stop`.  The `Send Msg` button cab be used to mail the message.
...
the MCI node if different from what is specified by `station`.
...
This should be commented out, if there is no MCI node or the MCI
returns data in the standard position, `2`. This exists to support
early MCI nodes that returned the data in the `3` position.
