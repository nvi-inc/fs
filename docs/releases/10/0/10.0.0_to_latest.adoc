...
part. It is more involved and can be deferred to a later date. The
remainder of this change item describes the second part.
...
Depending on the size of the _.rxg_ files, cleaning up the trailing
...
to clean them up, you can either keep them in the backup you made or
append all the trailing comments from the backups into the
corresponding _.rxg_ files. Future updates with _gnplt_ will add
...
possible to automate this step. If you are would like to have a script
to implement this procedure, please mail Ed. If there is sufficient
...
Using the following procedure to clean-up the trailing comments. As
_oper_, for each individual _.rxg_ file:
...
the `0` calibration set will be a commented out Tcal table entry
(with three fields after the leading `\*`, i.e.:
``*{nbsp}__polarization{nbsp}frequency{nbsp} Tcal__``). You should
find that line instead.
