...
most users. It will provide some benefit even if the second step is
never used. The first step makes a backup of your current _.rxg_ files
and removes all potentially extraneous comments. If the first step is
performed as part of this FS update, or at least before you next
update of your _.rxg_ files with _gnplt_, it will simplify your _.rxg_
file comments and will preserve the opportunity to perform the second
step as written at a later date.

+

The second step is more complicated and can be deferred until later.
It assumes you have only been updating your _.rxg_ files with _gnplt_;
in particular, you have not hand editing them. It uses the backed up
_.rxg_ files to make a minimal set of trailing comments and append
them to your active _.rxg_ files. If you have been hand editing your
_.rxg_ files, you may not be able to use the second step exactly as
written, but you may find it useful as a guide.

+

Please see the appendix <<Cleaning up .rxg file comments>> if you want to
apply this cleanup.
...
to read form. You can merged these comments, which are at the end of
...
by _gnplt_, it should work are written. If other modifications have
...
As described in <<stop_adding_extra_comments,Stop adding extra comment
is when updating .rxg files>> change in the <<FS changes>> sub-section
above, _gnplt_ has been adding extra comments, in some cases a lot of
them, to your _.rxg_ files. With this update, that behavior has been
fixed. However, there may already be many extra comments in your
_.rxg_ fies. This appendix provides an optional process for cleaning
up any extra comments. This cleanup is entirely optional. If it is not
performed, there will be no loss of functionality. There will just be
some extraneous comments in your _.rxg_ files.
