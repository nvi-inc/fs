...
command. This may not be a suitable solution in all stations. If
you have (or need) a local `lo` command you can continue to use it (or
implement one), but it will need to be updated to get new capabilities
such as new racks and <<logrxg,logging .rxg files>> when they come
along.
+
...
the <<mk6drudg,Mark 6>> item in the
...
schedule file. Additionally, _drudg_ only provides preliminary
support for Mark 6 recorders (see <<mk6drudg,Mark 6 recorder support>>
below) for use with RDBEs.  The features for broad band support are:
