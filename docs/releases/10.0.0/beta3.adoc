...
deprecated TCP comminication scripts, is only relavent if you are
updating from the VGOS branch, versions _9.12.x_, and probably only if
you had a RDBE backend.
...
sufficient for normal FS for VGOS observations if the schedule
procedure library is provided by a specially crafted _.skd_ schedule
file (see the <<drudg changes relative to the main branch>>
...
is sufficient for normal FS for VGOS observations if the schedule
procedure library is provided by a specially crafted _.skd_ schedule
file (see the <<drudg changes relative to the main branch>>
sub-section below for more details).
included ares
...
The details of cf Mark 6 features are described in the
...
configuration setting of a FiLa10G attached to the second DBBC2.
+

This provides enough flexibility for VGOS schedules using the PFB
personality of the DBBC2s. Currently the schedule procedures for  VGOS
operations are provided directly from the VGOS _.skd_ schedules. 
+
...
Th _s_client_ script should be used to replace several other similar,
...
apparently was relatively benign. Incorrect behavior was probably
limited to using leading zeros for some fields.
...
covers FS changes that occurred in the main branch between _9.11.2_
and _10.0.0_. These changes are covered extensively in several update
notes in the _misc/_ subdirectory:
...
. Add a time-out feature in the `onsouce` command to enable waiting
...
. Add support for DBBC2 DDC with continuous calibration.
...
. Add support for the DBBC2 PFB personality.
...
. Effective support for _broad band_ systems
+
...
for Mark 6 recorders. The features for broad band support are:
...
This block holds verbatim copies of the procedures for each station.
They are used if the rack type in _drudg_ is set to `BB`. For this
case, with the recorded selected as `Mark6`, the setup procedure's
name in the _.snp_ file is `setupbb`.
+
...
* Mark 6 recorder support
+

The _drudg_ Mark 6 support is selected by using the `Mark6` recorder
type in _drudg_. It is implemented using low-level `mk6` device
communication commands in the _.snp_ file. These are direct commands
to the recorder using its communication protocol. It is assumed that
the recorder had been initialized and modules mounted and grouped
before the schedule starts. _drudg_ inserts commands at the following
...
between _9.11.2_ and _10.0.0_. These changes are covered extensively
...
. Fix for a bug that caused the IF configuration to be lost if the schedule
specified an unrecognized rack type.

. Allow different stations to use different BWs.

. Avoid failing if some stations have unsupported rack and recorder types in
the schedule.
...
. Make _drudg_ backward compatible with old DBBC2 DDC rack types,
treating them as DDC.
