...
the FS as well as on 64-bit systems. However, the display server
currently does not work on FSL8 (_Wheezy_), and earlier, systems. The
update notice includes instructions for transitioning to a 64-bit
system, which may require some relatively simple changes to station
software. See the <<../../misc/64-bit_conversion.adoc#,64-bit
conversion>> document for more details.
...
=== Other upgrade documents

Upgrading to the <<beta3_to_latest.adoc#,latest commit from beta3>>.

Upgrading to the <<latest_commit.adoc#,latest commit>>.

Upgrading to <<beta2.adoc#,beta2>>.
...
CAUTION: The _beta3_ pre-release of the FS wll not _build_ on older
Linux distributions, such as FSL8 (_Wheezy_), unless the display
server is removed from the build. There is a reference to how to
remove the display server from the build in sub-step <<Make the FS>>
below.
...
sub-step below and continue with the remaining steps.
+
...
If you use the _fesh_ script to fetch schedules, and optionally run
_drudg_ for them, there are two changes:
...
display server, you can also skip this. You should not implement this
step, or removing setting the variable, if you built the FS without
the display server as described in <<noserver, not building the
display server>>.
...
. Update release model document <<release_model,More details>>.

. Add environment variables document <<env_vars,More details>>.
...
removed from the _fserver/tests/convey.*_ files.
...
.. Remove usage of `system()` call to find `help` files (closes
...
. [[release_model]] Update release model document.

.. The release steps were clarified.

.. The document was converted to _.adoc_ format and updated for the
new _.adoc_ scheme. It is viewable at
<<../misc/release_model.adoc#,Release Model>>.

. [[env_vars]] Add environment variables document. This document
describes user settable environment variables for both _make_ time and
runtime.  It is viewable at
