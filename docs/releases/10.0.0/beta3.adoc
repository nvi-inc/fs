...
covers the changes that were introduced in VGOS branch and have been
incorporated into the main branch as of _10.0.0_.
...
. [[sclient]] New _s_client_ external program for TCP device
communication.
+

This utility is a generalization of Haystack's _be_client_ script.
Errors are reported as `lg   -1` errors in the log if the FS is running.
The details on its use are available from `help=sy`.
+

This command should be used to replace several other similar, new but
deprecated, TCP scripts from the VGOS branch: _be_client_, _mcicn_,
and _udceth0_.
...
This update merges the VGOS branch into the main branch. The VGOS
branch diverged from the main branch at _9.11.2_. This sub-section
covers _drudg_  changes that were introduced on VGOS branch and have
been incorporated into the main branch as of _10.0.0_.
