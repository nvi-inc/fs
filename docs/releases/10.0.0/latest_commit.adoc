...
https://github.com/nvi-inc/fs/commit/746f40d670a36093ff613167139e4e2c77afae56[746f40d6].
...
NOTE: For FSL8 and other old Linux distributions, access to
_cddis_ may not be possible, due to out of date certificates.  In that
case, _bkg_ or _opar_ may be suitable alternatives.
...
WARNING: The FTP-SSL method may not work from behind some firewalls.
If it doesn't work for you, either use a different data center (see
...
NOTE: The use of FTP-SSL by _cddis_ may be deprecated in the future.
...
WARNING: _Let the user beware._ This feature must be used with extreme
care.  The answers that are specified must correspond exactly to the
questions that _drudg_ will ask. If they don't correspond correctly,
_drudg_ may produce subtly incorrect output with no obvious indication
of a problem. The _fesh_ script does what consistency checking it can,
e.g., if `FESH_CONT_CAL` is specified as `off`, no answer can be
supplied for `FESH_CONT_CAL_POLARITY` since that question will not be
asked. It is important to verify that correct output is being
produced.

+

WARNING: The feature will not work for schedules that have more than
...
.. Reorganized as _.adoc_ files in the _docs/_ sub-directory  and
available as web pages at: https://nvi-inc.github.io/fs/. Hopefully,
this will make the update notes easier to read and navigate. Among
other improvements, there are clickable links to other sections within
a document as well as to other related documents.
