...
<<64-bit_conversion.adoc#,64-bit conversion>> document for more details.
...
Converting to a <<64-bit_conversion.adoc#,64-bit system>>.
...
<<64-bit_conversion.adoc#,Converting to a 64-bit system>> document.
To accommodate that, there are a few embedded
comments for handling 64-bit systems. Otherwise the instructions are
...
NOTE: If you are following this step on your
32-bit system as part of the process in the 
<<64-bit_conversion.adoc#,Converting to a 64-bit system>>
docment, you may prefer to only do a quick check now and
...
changes.
[[details]] Each summary item has a clickable <<details,More details>>
link that leads to the detailed description of that item.

Each sub-section is divided further into three sub-sections:

. Changes that are in common since FS9
+
These sub-sections cover changes compared
to both FS9 branches. These are changes that have been introduced in version _10.0.0_.

. Changes relative to the main branch (9.13.2)
+
These sub-sections cover changes that are only relative to the main
branch. These are primarily features that were in the VGOS branch but
not in the main branch.

. Changes relative to the VGOS branch
+
These sub-sections cover changes that are only relative to the VOGS
branch.  These are primarily features that were in the main branch but
...
This sub-section is divided further into three sub-sections. Please
see <<Changes from previous versions>> above for an explanation of the
division.
...
.  Use of the FS display server is now recommended for normal use. <<useserver,More details>>.
...
. Make use of the FS display server the default. <<server2,More details>>.
...
<<64-bit_conversion.adoc#_handling_c_station_code,Handling C station code>>
section of the
<<64-bit_conversion.adoc#,Converting to a 64-bit system>> document.
...
. [[useserver]] Use of the FS display server is now recommended for normal use.
...
participate. Within the FS proper, only _drudg_ and its libraries
...
for all back-ends. The fact that this changed relative to the main branch
is is now noted in <<beta2.adoc#_changes_from_previous_versions,Changes from previous versions>> section
of <<beta2.adoc#,beta2 update notes>> document.
...
. [[server2]] Make use of the FS display server the default. This was changed as
of the _beta2_ release.
...
.. Some conditional changes in
the <<beta2.adoc#\_update_control_files,Update control files>>
step of the <<beta2.adoc#,beta2 update notes>> document
were corrected to properly
depend or not depend on the old version being _9.12.12_. If you
installed the _beta1_ version and your old version was _9.12.12_
you should review the instructions in
<<beta2.adoc#_update_control_files,Update control files>>
step to verify everything has been covered.

.. A sub-step was added to make using the FS display server the default.
If you installed _beta1_ you should check the 
the <<beta2.adoc#_set_fs_display_server,Set FS_DISPLAY_SERVER>>
...
and _prog_. If you installed _beta1_ you should check the
...
installing the latest version of _go_ as described in the
...
If you used that method it is recommended that you
