...
= Converting to a 64-bit system with upgrading to 10.0.0
...
10.0.0 and 64-bit without disturbing you operational 32-bit system.
...
Working on the new system, login as _prog_.

=== Update station code

IMPORTANT: All work is to be done on the new system.

There are two sub-steps. The first is for that covered by the 
 <<beta2.adoc#,FS update>> document. The second is to convert any C language code.

==== 10.0.0 code update steps

Follow the <<beta2.adoc#_update_station_programs,Update station programs>>
step from the <<beta2.adoc#,FS update>> document.
 
NOTE: If
you use FORTRAN in your station code you must convert to using _f95_.
...
Once your code __make__s successfully, you can continue, but you may need to debug
it when testing ih <<Follow remaining 10.0.0 update steps>> step below.


=== Follow remaining 10.0.0 update steps

Continuing to work on the new computer, follow the remaining steps in the 
<<beta2.adoc#,FS update>> document starting with 
<<beta2.adoc#_make_local_software,Make local software>> step.

NOTE: You may need to debug your station code in the 
<<beta2.adoc#_test_the_fs,Test the fs>> step
