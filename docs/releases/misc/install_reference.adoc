...
You can use cut-and-paste (technically copy-and-paste) to reduce the
amount of typing involved in the installation. This reduces the
chances of missing required spaces and other easily missed characters
...
connected to the FS computer. You can use the normal cut-and-paste
...
. Run the _xwininfo_ program from an shell prompt in an _xterm_.
...
NOTE: Using this method with differently named _.Xresource_ files, it
possible to have customized window layouts for different displays and/or
users.
