...
with the FS or the firmware is _v124_ (no time is available in the multicast
for that version, so the multicast arrival time is shown). The epoch is `--`
for now since the VDIF epoch is not available yet in the multicast.  The
`DDBC3-FS` time difference, in seconds, is shown in inverse video if it is not
zero (positive if the DBBC3 time is later than the FS). The `DDBC3-FS` time
difference is `------` for firmware _v124_.
