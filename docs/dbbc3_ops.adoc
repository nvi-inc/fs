...
NOTE: The FS hard codes a VDIF frame size of `8000`. If a different size is
needed, please see the <<Handling other VDIF frame sizes>> appendix.
...
== Handling other VDIF frame sizes

The value of `8000` for the VDIF frame size is hard coded in the FS for the
DBBC3 and _jive5ab_ (and DBBC2/FiL10G as well). Currently, this is the correct
value, but some day in the future, different values may be needed. If that
occurs before the FS is updated to accommodate other values, this section gives
a recipe for handling it for the DBBC3 and _jive5ab_ until there is a FS
update. It is a little complicated, but should work. Hopefully, the FS will be
updated before it is necessary.
...
After commanding the new VDIF frame size, the FS will complain that the DBBC3
`vdif_frame` payload is not correct when you check the DBBC3 configuration
(i.e., using the `setup__mode__` procedure without the `force` parameter), but
if that is the only complaint, there should not be a problem.
...
These examples use `8200` as an example different frame size.
...
recorded in the previous section, but with the new frame size, for example:
...
the new frame size, to the `setup__mode__`  procedure (`setup01` in this
...
NOTE: The reason the setting of the VDIF frame size is done _outside_ of the
