...
static int dbbc3_mcast_arrival=92;
static int no_mcast_since_restart=0;
static int epoch_inserted=0;
static int version_correct=1;
...
{
    printw("Arrival ");
    if(no_mcast_since_restart) {
        printw("    ");
    } else {
        if(tsys_cycle->hsecs/100>dbbc3_mcast_arrival/100)
            standout();
        buf[0]=0;
    ...
    printw(" Age");
    if(no_mcast_since_restart && time <= 0)
        age=seconds-no_mcast_since_restart;
    int hours=age/3600;
    ...
    for(i=0;i<9;i++) {
        if(buf[i]==' ')
            ...
            if(age>dbbc3_mcast_arrival/100)
                standout();
            printw("%s",buf+i);
            break;
        }
...
{
    if(!no_mcast_since_restart) {
        if( ! version_correct && it1%2)
            standout();
        if(ifs_to_display<=2) {
            char temp[sizeof(version)];
            memcpy(temp,version,sizeof(temp));
            if(strlen(temp)>24)
                strcpy(temp+21,"...");
            printw("%.24s",temp);
    ...
    for(k=0;k<rows;k++) {
        ...
        /* legitimate times start at the first VDIF epoch */
            ...
            if(ifc.time > 0) {
              ...
              if(ifc.time_error>=-dbbc3_mcast_arrival/100 && ifc.time_error <=0)
            ...
            if(ifc.time <= 0 || NULL == ptr) {
                ...
                int tm_bad = time_error<-dbbc3_mcast_arrival/100 || time_error>0;
                tm_bad = tm_bad || age >dbbc3_mcast_arrival/100;
            ...
            if(!epoch_inserted) {
                printw("%3s","---");
            } else if(no_mcast_since_restart) {
                printw("%3s"," ");
            } else {
                buf[0]=0;
            ...
            if (!ifc.time_included) {
                printw("-----");
            } else if(no_mcast_since_restart || ifc.time <= 0) {
                printw("%5s"," ");
            } else {
                int time_error=ifc.time_error;
                int tm_bad = time_error<-dbbc3_mcast_arrival/100 || time_error>0;
                if(time_error>=-dbbc3_mcast_arrival/100 && time_error<0)
                    time_error=0;
                buf[0]=0;
            ...
            move(irow++,icol+9);
            if(no_mcast_since_restart || ifc.lo<0.0)
                printw("   ");
            else {
                if(ifc.pol==1)
                    printw("(R)");
