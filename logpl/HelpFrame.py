...
In the bottom of the Plot Details box is a row of buttons. They are Zoom out, Autoscale, and Clear changes. Plots are zoomed by clicking and dragging the right mouse button over the area the user wants to zoom in on and releasing the button. The plot is fully zoomed out when clicking the Zoom out button. The user may delete data points from a plot by double clicking on them with the right mouse button. The data point is then filled in white. When clicking the Autoscale button, this data point is then moved to the lower or upper border of the plot, and the entire plot is scaled without this point. To move it back in to the plot, simply double right click it again and press Autoscale. To clear all changes made to the plot (i.e., zooming or deleting points) click Clear changes and the plot will plotted in its original version.
"""
