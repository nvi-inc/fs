...
/* Include files */
#include "../include/stparams.h"     /* ST parameters            */
#include "../include/stcom.h"        /* ST shared mem. structure */
#include "../include/stm_addr.h"     /* ST shared mem. pointer   */
...
/* Handle each mode in a separate section */
  ...
  switch (imode) {
    ...
    case 0:             /* initialize */
      ierr = 0;
      buf = "Initializing antenna interface";
      logit(buf,0,NULL);
      fs->ionsor = 0;
      break;
    ...
    case 1:             /* source= command */
      ierr = 0;
      buf = "Commanding to a new source";
      logit(buf,0,NULL);
      fs->ionsor = 0;
      break;
    ...
    case 3:        /* onsource command with error message */
      ierr = 0;
      buf = "Checking onsource status, extended error logging");
      logit(buf,0,NULL);
      fs->ionsor = 1;
      break;
    ...
    case 4:            /* direct antenna= command */
      ...
      nrecr = 0;
      for (i=0; i<nrec; i++) {
        buf2="Received message for antenna: ");
        nchar = cls_rcv(class,buf,sizeof(buf),&r1,&r2,dum,dum);
        buf[nchar+1] = NULL;  /* make into a string */
        strcat(buf2,buf);
        logit(buf2,0,NULL);
        buf="ACK";
        clasr = 0;
    ...
    case 5:    /* onsource command with no error logging */
      ierr = 0;
      buf = "Checking onsource status, no error logging");
      logit(buf,0,NULL);
      fs->ionsor = 1;
      break;
    ...
    case 7:    /* onsource command with additional info  */
      ierr = 0;
      buf = "Checking onsource status, log tracking data");
      logit(buf,0,NULL);
      fs->ionsor = 1;
