...
class Gui(Frame):
    ...
    def __init__(self, parent = None, **args):
        ...
        scalingmenu.add_radiobutton(label = 'Autoscale not including deleted', variable = Gui.scaling_mode, value = 0, command = lambda: self.scale('without_deleted'))
        ...
        if self.selectedX.get() and self.selectedY.get():
            ...
            if data_indices:
                ...
                try:
                    ...
                    status = "Error: The selected dataset's LO spans several RXG-files. Tools not available"
                    self.status.set(status)
                else:
                    #set tools
    ...
    def cleanUp(self):
        ...
        for orig_rxg in RXG_LIST:
            ...
            if os.path.isfile(working_rxg):
                #working file exists, ask the user to replace
