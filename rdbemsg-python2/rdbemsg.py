...
#			print v
	...
	def GetWXVals(self):
		...
		s = self.sessionname.get()
		c = self.stationcode.get()
		procWX = subprocess.Popen(["/bin/sh", "-c", "grep 'wx/' /usr2/log/" + s + c + ".log  | tail  -n 1"], stdout=subprocess.PIPE)
	...
	def sendMessage(self):
		...
		subprocess.Popen(["inject_snap", '"' + SUBJECT], stdout=FNULL, stderr=FNULL)
		TEXTlist = TEXT.splitlines()
		[subprocess.Popen(["inject_snap", '"' + line], stdout=FNULL, stderr=FNULL) for line in TEXTlist]
