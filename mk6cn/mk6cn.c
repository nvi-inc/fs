 ...
 */
/* mk6cn: based on John Ball's tstMark5A of 2002 February 18 */
...
int main(int argc, char * argv[])
{
  ...
  strcat(control_file,"/control/mk6");
...
int doinit()
{
...
#ifdef DEBUG
        printf("cannot open MK6 address file %s\n",control_file);
#endif
        return 0 ;
    }
...

    {
      ...
      for(i=0;i<MAX_MK6;i++)
	if(me[4]==lets[i]) {
	  shm_addr->mk6_units[i]=1;
	  shm_addr->mk6_active[i]=1;
	}
    }
    ...
    is_init=TRUE;
    if (0 > (error = open_mk6(host,port))) { /* open mk6 unit */
#ifdef DEBUG
      printf ("Cannot open mk6 host %s port %d error %d\n",host,port,error);
#endif
      fail=FALSE;
      return error;
    }
...
/* ********************************************************************* */
int open_mk6(char *host, int port)
{   
  ...
  /* * Get service number for socket m6drive * */ 
  socaddin.sin_port = htons(port); /* Port m6drive's number */
#ifdef DEBUG
  (void) fprintf(stderr, "%s DEBUG:  m6drive port is %d \n", 
...
int ip[5];
{
  ...
  for (i=0;i<in_recs;i++) {
    ...
    if(ptr==NULL || (ptrcolon!=NULL && ptr >ptrcolon) ||
      ...
      /* are there trailing parameters that could contain error info */
      ptr=strchr(outbuf,':');
      if(ptr!=NULL) {
	char *ptr2;
	...
	if(*ptr!=0) {
	  ...
	  strcpy(save2,"mk6");
	  strncat(save2,who,2);
	  strcat(save2,": Mark6 error information: ");
...
////////////////////////////////////////////////////////////////////////////////////
// close_socket: cleanly close the TCP socket, can be used _after_ a open_mk6()
////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
// read_response:
//  Reads a single Mk6 response delimited by a newline.
...
//    draining the input buffer before the next write to the Mark 6
