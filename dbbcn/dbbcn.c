...
#define CONTROL_FILE "/usr2/control/dbbad.ctl"
...
{   
  ...
  if(signal(SIGALRM,nullfcn) == SIG_ERR){
    fprintf( stderr,"dbbcn: setting up signals, gethostbyname()\n");
    exit(-1);
  }
  rte_alarm( time_out);
  ...
  if(signal(SIGALRM,SIG_DFL) == SIG_ERR){
    fprintf( stderr,"dbbcn: setting default signals, gethostbyname()\n");
    exit(-1);
  }
...
#ifdef DEBUG
      printf ("dbbcn failed to get a request buffer\n");
#endif
      ip[2] = -101;
      goto error;
    }
