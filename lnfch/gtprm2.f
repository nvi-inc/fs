      subroutine gtprm2(ibuf,ifc,iec,itype,parm,ierr)

C     get next parameter
C 
C        GTPRM2 parses the input buffer and returns the next parameter 
C        GTPRM2 is just like GTPRM, but it return an (unambiguous) error
C               code for a comma or asterisk value
C 
C     INPUT VARIABLES:
C 
...
C                 < 0 is an error
C                 0 normal return, PARM is valid
C                 1 '*' (current) was the value
C                 2 ',' (default) was the value
