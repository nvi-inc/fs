...
[Field System](https://github.com/nvi-inc/fs), but for now there are a couple of
ways to obtain it. Installation should probably be carried out under the
superuser account
...
* Force an update to the schedule file when there's a new one available to
  ...
  drudged if the DoDrudg option is True (default = False)
