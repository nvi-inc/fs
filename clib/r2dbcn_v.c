/* r2dbe detector queries for fivpt */
/* two routines: r2dbcn_d identifies the detector to be sampled */
/* r2dbcn_v samples it */
/* call r2dbcn_d first to set-up sampling and then rdbcn_v can be */
...
void r2dbcn_v(double *dtpi, double *dtpi2, long ip[5], int *icont, int *isamples)
{
  ...
  if(0!=r2dbe_2_tsysx(buf,&ifcr,ip,on,off,who)) {
    return;
  }
  *dtpi=off[ichan];
  *dtpi2=on[ichan];
