...
230   continue
      ...
      if (ichcm_ch(parm,1,'fs').eq.0) irep = 0 
      if (ichcm_ch(parm,1,'by').eq.0) irep = 1 
      if (ichcm_ch(parm,1,'rw').eq.0) irep = 2 
      if (ichcm_ch(parm,1,'ab').eq.0) irep = 3 
      if (cjchar(parm,1).eq.',') irep = 0
C                 Default is FS mode
      if (cjchar(parm,1).eq.'*') irep = ireppc 
      iby = 1 
      if (irep.eq.2) iby = 0
...
240   continue
      call gtprm(ibuf,ich,nchar,1,parm,ierr) 
      if (cjchar(parm,1).eq.',') nblk = 3
C                 Default is 3 SECONDS INTEGRATION TIME
...
cxx      write(6,8888)
cxx8888  format(10x,"time to run PCALR from PCALS",/)
      call run_prog('pcalr','nowait',ip(1),ip(2),ip(3),ip(4),ip(5))
C      Schedule PCALR 
990   ip(1) = 0 
      ip(2) = 0 
      ip(3) = ierr
