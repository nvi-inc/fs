...
C
      ...
      character*4 caxfp
C
C---- Set variables for data plot subroutine (latlon).
      real lttim(31),lttemp(31),ltoff(31),ltplpar(5),
     &     lntim(31),lntemp(31),lnoff(31),lnplpar(5)
C
...
C
      do i=1,abs(nptsfp)
        ...
        lttemp(i)=temp(i) ! Stock for plot routine
        tim(i)=tima
        lttim(i)=tim(i)   ! Stock for plot routine
        sig=sig*vslope
        off(i)=latoff
        ltoff(i)=off(i)   ! Stock for plot routine
C
...
C
C---- Stock Latfit data for plot routine.
      do i=1,5
        ltplpar(i)=ltpar(i)
      end do
C 
...
C 
      do i=1,abs(nptsfp) 
        ...
        if (ierr.ne.0) goto 80010
        temp(i)=(tpia-vbase)*vslope-temps
        lntemp(i)=temp(i) ! Stock for plot routine
        tim(i)=tima
        lntim(i)=tim(i)   ! Stock for plot routine
        sig=sig*vslope 
        off(i)=lonoff
        lnoff(i)=off(i)   ! Stock for plot routine
C 
...
C
C---- Stock Lonfit data for plot routine.
      do i=1,5
        lnplpar(i)=lnpar(i)
      end do
C
...
C
C---- Plot observation data and its model.
      call latlon(nptsfp,ltplpar,lttim,lttemp,ltoff,
     &                   lnplpar,lntim,lntemp,lnoff,coslat)
      iopen=1
C
