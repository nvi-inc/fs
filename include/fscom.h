...
/*        struct ifatt_shm ifatt; This will be used in the future.*/
	...
	float cablevl;
        ...
        struct onoff_cmd_dummy_10_0_0 onoff_dummy_10_0_0;
  ...
  struct rdbe_quantize_cmd rdbe_quantize[MAX_RDBE+1];
  ...
  struct r2dbe_tsys_data {
    struct r2dbe_tsys_cycle {;
      char epoch[14];
      int epoch_vdif;
      float tsys[MAX_R2DBE_CH+2][MAX_R2DBE_IF];
      float pcal_amp[MAX_R2DBE_CH*32];
      float pcal_phase[MAX_R2DBE_CH*32];
      float pcal_offset;
      float pcal_spacing;
      int pcal_ifx;
      int tone0;
      float sigma[MAX_R2DBE_IF];
      double dot2gps;
      double dot2pps;
      time_t arrival;
    } data[2];
