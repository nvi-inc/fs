     ...
     .                 mask_cat,sequence_cat,lo_cat,head_cat,hdpos_cat,
     .                 tracks_cat,flux_cat,vlba_cat,flux_comments,
     .                 csked,cdrudg,
...
C  $CATALOGS
            if (ichcm_ch(isecname,1,'$CATALOGS').eq.0) then 
              do while((ilen.ne.-1).and.(jchar(ibuf,1).ne.o'44')) 
                ...
                else if (ichcm_ch(itmpnam,1,'SEQUENCE').eq.0) then
                  call hol2char(ibuf,ic1,ic2,sequence_cat) 
                else if (ichcm_ch(itmpnam,1,'LO').eq.0) then
                  call hol2char(ibuf,ic1,ic2,lo_cat) 
                else if (ichcm_ch(itmpnam,1,'HEAD').eq.0) then
                  call hol2char(ibuf,ic1,ic2,head_cat) 
     ...
     .            then
                ...
                else if (ichcm_ch(itmpnam,1,'VLBA').eq.0) then
                  call hol2char(ibuf,ic1,ic2,vlba_cat) 
                else
