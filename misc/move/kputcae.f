...
*
      subroutine kputcae(names,cra,cdec,cepoch,icount,max_src,ra_plus,
     &                   oaz,oel)
      implicit none
...
c
      if(icount.gt.0) then
        write(6,
     &   "(' source',13x,'R.A.(hms)',7x,'Dec.(dms)',5x,'Epoch',3x,
     &     'Az.(deg)  El.(deg)')")
      endif
      do i=1,icount
        cdum=cra(i)*RAD2SEC
...
c
        ...
        az=oaz(i)*RAD2DEG
        el=oel(i)*RAD2DEG
        write(6,'(a,a,2i3,f7.3,3x,a,2i3,f6.2,f10.3,2f10.5)')
     &  name, sra, rah, ram, ras, sdec ,decd, decm, decs, epoch, az, el
      enddo
c
      end
