...
C Local
      ...
      integer*2 ibuf(ibuf_len),lsn(4),lfrq,lstart(6)
...
C    Convert start time to ascii.
      ...
      idum = ichmv(lsn,1,lsorna(1,isor),1,8)
      lfrq = lcode(icod)
      nch=1
...
C       if (ichcm(lsn,1,lskobs(1,ir),ic1,8).eq.0) then ! continue
        if (ichcm(lsn,1,lskobs(1,ir),1,8).eq.0) then ! continue
C       CALL IFILL(LSN,1,8,oblank)
C       IDUM = ICHMV(LSN,1,IBUF,IC1,MIN0(IC2-IC1+1,8))
